...
//メンバ関数実体
//コンストラクタ
template <class TEMPLATE> inline _vec<TEMPLATE>::_vec(void):x(0),y(0),z(0){}

template <class TEMPLATE> inline _vec<TEMPLATE>::_vec(const TEMPLATE& a, const TEMPLATE& b, const TEMPLATE& c):x(a),y(b),z(c){}

template <class TEMPLATE> inline _vec<TEMPLATE>::_vec(const _vec<TEMPLATE>& v):x(v.x),y(v.y),z(v.z){}
...
//-----------------------------------------------------------------------------------------------
//DEJI add a postscript
#include<iostream>
//using std::ostream;
template <class T> inline std::ostream& operator<< (std::ostream& stream,const _vec<T>& vec){
